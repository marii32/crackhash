...
//        List<WorkerTask> inProgressTasks =
//                workerTaskRepository.findByStatus(TaskStatus.IN_PROGRESS);
//        for (WorkerTask task : inProgressTasks) {
//            if (now - task.getUpdatedAt().getTime() > TASK_TIMEOUT) {
//                logger.warn("Task timeout: {}", task.getPartNumber());
